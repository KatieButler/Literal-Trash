...
function FinalAnimation () 
 ...
 %% DivGrad - Grad*V    
     ...
     [t,concentrationArray] = ode45(@Divgrad2, linspace(0,99), U20); %Taking ODE45 to solve the differential equation.
     ...
     function outcome = Grady(t, stateVector) %Setting up the grad function so we can take grad of the outcome of divgrad - gradV
        ...
        arbitrary = -gradU(xIndex,yIndex) + [u(xIndex, yIndex);v(xIndex,yIndex)];  %Adding velocity to -grad.
    ...
    comet(y1(:,1), y1(:,2)); %Trying to animate the outcome as a vector field.
end 
