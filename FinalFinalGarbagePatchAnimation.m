function FinalFinalGarbagePatchAnimation () 
  %%Sources
...
%% V 
    ...
    source = (1*exp(-20*((xTemp-1).^2 + (yTemp - 1).^2)))+ (1*exp(-20*((xTemp + 1).^2 + (yTemp + 1).^2)));
    %disp(source)
 ...
 %% DivGrad - Grad*V    
     ...
     function res = Udipole2(t,x,y) %Setting up the initial density field.
         res = 5.1*[-0.5* log(((x - 1)^2 + (y - 1)^2)) + -0.5* log(((x + 1)^2 + (y + 1)^2))]; %This is the equation for it.
     end
